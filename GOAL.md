...
- [x] **M18–M19 — ciclo de processos e standby:** freeze, reclaim seguro e retomada possuem cobertura de host e validação QEMU, incluindo standby persistente de userspace; ainda falta exercitar pressão prolongada e recuperação em hardware real.
- [ ] **M20–M23 — interface do sistema:** parcial. O compositor continua responsável apenas por surfaces, foco, input, damage e apresentação; a UI file-backed possui `/system/ui/variables.conf`, providers somente leitura, manifesto seguro, HTML/CSS externo e fragmentos editáveis para desktop, status, topbar, dock, launcher, Alt+Tab, wallpaper, widgets, notificações, mídia, terminal e ações rápidas. Status/topbar/ações já não ficam duplicados no template-base; ações são allow-listed e possuem capabilities. A prova Zig exercita surfaces independentes de desktop/aplicação, input, present, resize, foco e destruição; `ui_ipc_loopback.zig` prova o envelope wire e o lifecycle no limite do transporte; a prova C mantém compatibilidade ABI (`tools/test-ui-slice.ps1`). O runtime HTML próprio permanece fallback/bootstrap. A migração do shell visual hardcoded para o engine userspace real e ligação de todos os eventos ao gerenciador ainda continuam.
...
O parser de `run` também preserva argumentos agrupados por aspas, permitindo
invocar applets com texto contendo espaços.
Foi criado o primeiro runtime HTML sem alocação dinâmica (`graphics/html.zig`),
...
`RESET`, `TERMINAL`, `MONITOR` e `FILES`; Enter ativa o controle selecionado.
Os botões de aplicação abrem ou focam janelas reais pelo window manager, com a
listagem FILES atualizada ao ser aberta, e o estado HTML `ONLINE` permanece
visível na mesma janela.
...
painel visual e publica o menu selecionado no serial. `Esc` fecha o menu aberto.
Clique fora da barra também dispensa o painel automaticamente.
