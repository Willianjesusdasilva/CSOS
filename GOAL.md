...
- [ ] **M14 — GPU AMD/NVIDIA + Vulkan:** parcial. A preparação AMD GFX11, a ABI DRM, capacidade PCIe combinada, contrato GPUVA de 48 bits, clocks ATOM e snapshots físicos de CUs/RBs ativos possuem implementação e testes de host, mas tipo/largura de VRAM e campos posteriores, execução Radeon real e triângulo RADV ainda faltam. NVIDIA/NVK vem depois do primeiro triângulo AMD e também exige validação real.
...
1. Completar tipo/largura de VRAM e demais campos físicos do `AMDGPU_INFO_DEV_INFO`, command submission e RADV no caminho AMD GFX11.
