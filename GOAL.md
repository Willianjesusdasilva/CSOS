...
- [ ] **M15 — SDL:** parcial. Foi criado o contrato software inicial (`graphics/sdl.zig`) com superfície RGBA, janela, fila de eventos quit/teclado/mouse (incluindo roda) e dispositivo de áudio com validação de especificação, coberto por teste host; o loop HID do kernel já converte teclado/mouse para essa fila e `display.blitSurface` copia superfícies SDL ao backbuffer, faltando compositor SDL completo, integração do áudio com hardware e aplicações SDL reais.
