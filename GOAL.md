...
- [ ] **M15 — SDL:** parcial. Foi criado o contrato software inicial (`graphics/sdl.zig`) com superfície RGBA, janela, fila de eventos quit/teclado/mouse (incluindo roda), `peek`/estado do último evento e dispositivo de áudio com validação de especificação, coberto por teste host; o loop HID do kernel já converte teclado/mouse para essa fila e `display.blitSurface` copia superfícies SDL ao backbuffer, faltando compositor SDL completo, integração do áudio com hardware e aplicações SDL reais.
...
`zig build test` conclui `15/15` etapas e `20/20` testes aprovados; um boot QEMU limitado também voltou a alcançar
...
Verificação mais recente em 2026-09-06: `zig build test` concluiu `15/15` etapas
e `20/20` testes, e o boot QEMU chegou a `CSOS graphical session ready`. A
