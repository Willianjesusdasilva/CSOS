...
- [ ] **M20–M23 — interface do sistema:** parcial. O framebuffer possui window manager/compositor software com criação/fecho de janelas, foco, hit-test, Alt+Tab, arrasto limitado à tela e composição por camadas, testável em QEMU; SDL já cobre superfícies/eventos e o runtime HTML renderiza `h1`/`p`/`div`/`span`/listas/`br`/`textarea`/checkboxes/`button`/links `a`, com classes, cores inline, foco, ativação, estado marcado e abertura das aplicações TERMINAL, MONITOR e FILES. FILES possui filtro FAT editável por teclado, preview e mutações de arquivo via ABI; ainda faltam CSS/Jinja completos, ligação de todos os eventos ao gerenciador, aplicações SDL reais e UI dinâmica mais ampla.
