...
- [ ] **M20–M23 — interface do sistema:** parcial. O framebuffer possui window manager/compositor software com criação/fecho de janelas, foco, hit-test, Alt+Tab, arrasto limitado à tela e composição por camadas, testável em QEMU; SDL já cobre superfícies/eventos e o runtime HTML inicial renderiza `h1`/`p`/`button`, com foco, ativação e abertura das aplicações TERMINAL, MONITOR e FILES. Faltam texto completo, CSS/Jinja, ligação de todos os eventos ao gerenciador, aplicações SDL reais e UI dinâmica mais ampla.
