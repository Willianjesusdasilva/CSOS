...
- [ ] **M14 — GPU AMD/NVIDIA + Vulkan:** parcial. A preparação AMD GFX11, a ABI DRM até MALL, capacidade PCIe combinada, contrato GPUVA de 48 bits, tipo/largura de VRAM via ATOM, clocks e snapshots físicos de CUs/RBs/TCC/UMCs ativos possuem implementação e testes de host, mas shadow/CSA/userq, execução Radeon real e triângulo RADV ainda faltam. NVIDIA/NVK vem depois do primeiro triângulo AMD e também exige validação real.
...
1. Completar shadow/CSA/userq do `AMDGPU_INFO_DEV_INFO`, command submission e RADV no caminho AMD GFX11.
