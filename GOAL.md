...
- [ ] **M20–M23 — interface do sistema:** parcial. O compositor continua responsável apenas por surfaces, foco, input, damage e apresentação; a UI file-backed agora possui `/system/ui/variables.conf`, providers somente leitura, manifesto de composição, HTML/CSS externo, fragmentos de desktop/topbar/dock/launcher/Alt+Tab, ações allow-listed e provas userspace C/Zig via `csos_ui_backend` (`tools/test-ui-slice.ps1`). O runtime HTML próprio permanece fallback/bootstrap. A migração do shell visual hardcoded para o engine userspace real, CSS/Jinja completos e ligação de todos os eventos ao gerenciador ainda continuam.
