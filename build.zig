...
pub fn build(b: *std.Build) void {
    ...
    const fd_table_test_module = b.createModule(.{
        .root_source_file = b.path("kernel/fd_table.zig"),
        .target = b.graph.host,
        .optimize = optimize,
    });
    const fd_table_tests = b.addTest(.{ .root_module = fd_table_test_module });
    const run_fd_table_tests = b.addRunArtifact(fd_table_tests);
    test_step.dependOn(&run_fd_table_tests.step);
    const apic_test_module = b.createModule(.{
        .root_source_file = b.path("arch/x86_64/apic.zig"),
        .target = b.graph.host,
        .optimize = optimize,
    });
    const apic_tests = b.addTest(.{ .root_module = apic_test_module });
    const run_apic_tests = b.addRunArtifact(apic_tests);
    test_step.dependOn(&run_apic_tests.step);
    ...
    const installer_state_test_module = b.createModule(.{
        .root_source_file = b.path("installer/state.zig"),
        .target = b.graph.host,
        .optimize = optimize,
    });
    const installer_state_tests = b.addTest(.{ .root_module = installer_state_test_module });
    const run_installer_state_tests = b.addRunArtifact(installer_state_tests);
    test_step.dependOn(&run_installer_state_tests.step);
    ...
    kernel_module.addAnonymousImport("ui_terminal", .{ .root_source_file = b.path("system/ui/interface/terminal.html") });
    kernel_module.addAnonymousImport("ui_status", .{ .root_source_file = b.path("system/ui/interface/status.html") });
