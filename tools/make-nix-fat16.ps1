...
$command = "set -eu; root=/tmp/csos-nix-fat-root; rm -rf `$root; mkdir -p `$root; tar -xf '$archiveWsl' -C `$root; mkdir -p '$outputDirWsl'; truncate -s ${SizeMiB}M '$outputWsl'; mkfs.fat -F 16 -S 512 -s 16 -n CSOSNIX '$outputWsl' >/dev/null; export MTOOLS_SKIP_CHECK=1; mmd -i '$outputWsl' ::/bin ::/lib ::/usr ::/usr/lib; mcopy -s -i '$outputWsl' `$root/usr/bin/nix ::/bin/; mcopy -s -i '$outputWsl' `$root/usr/lib/* ::/lib/; mcopy -s -i '$outputWsl' `$root/usr/lib/* ::/usr/lib/; mcopy -i '$outputWsl' `$root/lib/ld-musl-x86_64.so.1 ::/lib/; mdir -i '$outputWsl' ::/bin; mdir -i '$outputWsl' ::/lib | tail -n 1"
