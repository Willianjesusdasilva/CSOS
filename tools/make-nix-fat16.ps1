...
$command = "set -eu; root=/tmp/csos-nix-fat-root; rm -rf `$root; mkdir -p `$root; tar -xf '$archiveWsl' -C `$root; mkdir -p '$outputDirWsl'; truncate -s ${SizeMiB}M '$outputWsl'; mkfs.fat -F 16 -S 512 -s 16 -n CSOSNIX '$outputWsl' >/dev/null; export MTOOLS_SKIP_CHECK=1; mmd -i '$outputWsl' ::/nix ::/nix/bin ::/nix/lib; mcopy -s -i '$outputWsl' `$root/usr/bin/nix ::/nix/bin/; mcopy -s -i '$outputWsl' `$root/usr/lib/* ::/nix/lib/; mdir -i '$outputWsl' ::/nix/bin; mdir -i '$outputWsl' ::/nix/lib | tail -n 1"
