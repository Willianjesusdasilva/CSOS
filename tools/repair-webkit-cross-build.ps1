...
$expatLib = ((Join-Path $sysrootPath 'lib\libexpat.a').Replace('\','/'))
...
$dependencySuffix = " $gmoduleLib $pcre2Lib $ffiLib $expatLib $pixmanLib $pslLib"
Get-ChildItem $build -Recurse -File -Filter '*.rsp' -ErrorAction SilentlyContinue | ForEach-Object {
    ...
    if ($rspText.Contains('libglib-2.0.a') -and (-not $rspText.Contains('libexpat.a') -or -not $rspText.Contains('libpixman-1.a') -or -not $rspText.Contains('libpsl.a'))) {
