...
pub const Volume = struct {
    ...
    pub fn createDirectoryFile(self: *Volume, directory_cluster: u16, name: *const [11]u8) !void {
        try validateDataCluster(directory_cluster, self.cluster_count);
        var cluster = directory_cluster;
        var traversed: u32 = 0;
        while (true) {
            ...
            while (sector < self.sectors_per_cluster) : (sector += 1) {
                ...
                while (offset < 512) : (offset += 32) {
                    ...
                    if (entryIsAllocated(bytes + offset) and !entryIsLongName(bytes + offset) and equal11(bytes + offset, name)) return error.AlreadyExists;
                }
            }
            const next = try self.fatEntry(cluster);
