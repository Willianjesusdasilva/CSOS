...
fn hasSystemVersion(text: []const u8) bool {
    ...
    while (offset + prefix.len <= text.len) : (offset += 1) {
        if (offset != 0 and text[offset - 1] != '\n') continue;
        if (equalIgnoreCase(text[offset .. offset + prefix.len], prefix)) return true;
    }
    return false;
}
...
pub fn build(cpu: Cpu, facts: Facts) !Profile {
    ...
    for (cpu.vendor) |byte| if (byte < 0x21 or byte > 0x7e) return error.InvalidCpuIdentity;
