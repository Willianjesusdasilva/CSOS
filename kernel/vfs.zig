...
pub fn readLinkAt(directory_fd_in: i64, path: []const u8, output: []u8) !usize {
    if (equal(path, "/proc/self/exe") or equal(path, "proc/self/exe") or equal(path, "/proc/1/exe") or equal(path, "self/exe") or equal(path, "exe")) {
...
fn resolve(directory_fd: i64, path: []const u8) !Node {
    ...
    if (equal(path, "/home")) return .home;
    if (equal(path, "/data")) return .data;
    if (equal(path, "/system")) return .system;
    if (equal(path, "/tmp")) return .tmp;
...
fn nodeInfo(node: Node) Info {
    return switch (node) {
        .root, .bin, .dev, .dri, .sys, .sys_dev, .sys_char, .proc, .proc_self, .home, .data, .system, .tmp, .drm_char_primary, .drm_char_render, .drm_device, .drm_device_drm, .fat_directory => .{ .mode = 0o040755, .size = 0, .directory = true },
...
fn toFatName(path: []const u8) ?[11]u8 {
    ...
    if (std.mem.eql(u8, path, "terminal.html")) return "TERMINALHTM".*;
    if (std.mem.eql(u8, path, "status.html")) return "STATUS  HTM".*;
...
fn runtimeLibraryFatAlias(path: []const u8) ?[11]u8 {
    ...
    if (equal(name, "libnixfetchers.so")) return "NIXFETCHSO ".*;
    if (equal(name, "libnixflake.so")) return "NIXFLAK SO ".*;
    ...
    if (equal(name, "libcrypto.so.3")) return "LIBCRYP SO3".*;
    if (equal(name, "libsodium.so.26")) return "SODIUM  SO6".*;
    if (equal(name, "libbrotlidec.so.1")) return "LIBBROT SO1".*;
    if (equal(name, "libbrotlienc.so.1")) return "LIBBROE SO1".*;
    if (equal(name, "libbrotlicommon.so.1")) return "LIBBROC SO1".*;
    if (equal(name, "libbz2.so.1")) return "LIBBZ2  SO1".*;
    if (equal(name, "libcurl.so.4")) return "LIBCURL SO4".*;
    ...
    if (equal(name, "libseccomp.so.2")) return "LIBSECC SO2".*;
    if (equal(name, "libsqlite3.so.0")) return "LIBSQLI SO0".*;
    if (equal(name, "libgit2.so.1.9")) return "LIBGIT2 SO1".*;
    ...
    if (equal(name, "libnghttp2.so.14")) return "NGHTTP2 SO1".*;
    if (equal(name, "libidn2.so.0")) return "LIBIDN2 SO0".*;
    if (equal(name, "libpsl.so.5")) return "LIBPSL  SO5".*;
    if (equal(name, "libssl.so.3")) return "LIBSSL  SO3".*;
