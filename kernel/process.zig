...
fn protectMmap(address: u64, length: u64, writable: bool, executable: bool) callconv(.c) bool {
    ...
    // page-fault path commits individual pages when they are actually used.
    const noreserve_start: u64 = 0x000000c000000000;
    const noreserve_end: u64 = 0x00007f0000000000;
    if (address >= noreserve_start and address + length <= noreserve_end) {
        return true;
    }
...
fn mapAnonymous(address_space: *paging.AddressSpace, pages: *physical.Allocator, owned: *[max_owned_ranges]OwnedRange, owned_count: *usize, virtual: u64, count: u64) !void {
    if (count > std.math.maxInt(u64) / page_size or virtual > std.math.maxInt(u64) - count * page_size)
...
fn validateReclaimCandidates(mappings: []const Mapping, owned: []const OwnedRange) !void {
    for (mappings) |mapping| {
        ...
        if (mapping.physical < range.address or mapping.physical >= range_end or
            (mapping.physical - range.address) % page_size != 0) return error.MappingMismatch;
    }
}
...
test "standby reclaim preflight rejects a physical page outside ownership" {
    const owned = [_]OwnedRange{.{ .address = 0x4000, .pages = 1 }};
    var mappings = [_]Mapping{.{
        .virtual = 0x8000,
        .physical = 0x5000,
