...
pub fn start(info: BootInfo) noreturn {
    ...
    if (gpu_adapter.driver == .amdgpu and gpu_gmc11_nbio_registers != null and gpu_ip_discovery.?.gc_info != null and gpu_ip_discovery.?.mall_size != null and gpu_cu_info != null and gpu_gb_addr_config != null and gpu_clock_info != null and gpu_pcie_link != null and gpu_atom_vram_info != null and gpu_cache_info != null) {
        ...
        syscalls.configureAmdGpuInfoProfile(.{
            ...
            .mall_size = gpu_ip_discovery.?.mall_size.?,
        });
    ...
    while (true) {
        ...
        while (hid.pop()) |event| {
            if (event.kind == .keyboard) {
                ...
                if (monitor_shortcut_pressed) {
                    _ = launchDesktopWindow(window_manager, 2, &demo_app.window, &monitor_window, &system_window, &files_window) catch panic("desktop monitor shortcut launch failed");
                    serial.write("UI monitor shortcut\n");
                }
                if (system_shortcut_pressed or files_shortcut_pressed) {
                    const application_id: u32 = if (system_shortcut_pressed) 3 else 4;
                    _ = launchDesktopWindow(window_manager, application_id, &demo_app.window, &monitor_window, &system_window, &files_window) catch panic("desktop application shortcut launch failed");
                ...
                if (!terminal_shortcut_pressed and !monitor_shortcut_pressed and !system_shortcut_pressed and !files_shortcut_pressed and !launcher_consumed and !tab_switch_pressed and event.c != 0 and focusedWindowIs(window_manager, 4) and event.a != 0) {
                    if (event.a == 0x3e or (event.a == 0x15 and (event.b & 0x11) != 0)) {
                        ...
                        const changed = switch (event.a) {
                            0x4e, 0x2c, 0x28 => files_preview_pager.next(),
                ...
                if (left_pressed and !left_was_pressed) {
                    ...
                    if (desktopMenuHitTest(cursor_x, cursor_y)) |menu| {
                        ...
                        if (window_manager.closeHitTest(hit, cursor_x, cursor_y)) {
                            ...
                            } else if (window.id == 3 and window_manager.contentRectHitTest(hit, cursor_x, cursor_y, 4, 42, 64, 14)) {
...
fn statTerminalFile(path: []const u8, output: []u8) ?[]const u8 {
...
fn copyTerminalFile(source: []const u8, destination: []const u8) bool {
    const input = vfs.openAt(-100, source, 0) catch return false;
    defer vfs.close(input) catch {};
...
fn drawSystemSurface(window: *sdl.Window, storage_blocks: u64, input_devices: usize, audio_endpoints: usize) void {
    ...
    const document = if (system_html_active) html.Document.parse("<h1>CSOS SYSTEM</h1><button>ACTIVE</button><button class=danger>RESET</button><button class=accent>TERMINAL</button><button class=accent>MONITOR</button><button class=accent>FILES</button><p class=muted>ONLINE</p>") else html.Document.parse("<h1>CSOS SYSTEM</h1><button>READY</button><button class=danger>RESET</button><button class=accent>TERMINAL</button><button class=accent>MONITOR</button><button class=accent>FILES</button><p class=muted>ONLINE</p>");
